...
- Use the repo's confirmed model tiers only: `gpt-4o-mini`, `gpt-4o`, and `gpt-5.5`.
- v1 uses deterministic routing first so simple intent classification does not burn tokens.
- If an AI router is enabled later, use `gpt-4o-mini` through config and keep the prompt small.
- Do not introduce unconfirmed model names as placeholders.
...
- Deterministic v1 router: intent classification without token burn.
- `gpt-4o-mini`: future low-cost AI classification/summaries only if explicitly enabled.
- `gpt-4o`: normal chat, normal drafts, GHL/SMS first pass.
