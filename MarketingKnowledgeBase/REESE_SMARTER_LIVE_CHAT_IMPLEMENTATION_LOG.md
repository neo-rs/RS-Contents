# Reese Smarter Live Chat Implementation Log

## Purpose

This document records what was implemented for the smarter Reese live chat upgrade, what is real/live-ready now, and what remains future-only. It is meant to prevent stale placeholders from being forgotten in code.
...
RSAdminBot on_message
  -> confirm message is in Reese live chat channel
...
Future ticket/cancellation/suggestion intents are recognized only so Reese can say the workflow is not wired yet instead of guessing.
...
- Loads the shared Reese webhook profile for outbound webhook identity.
- Enforces smart-chat feature flags before running disabled workflows.
- Enforces per-intent token caps and max tool-round caps from config.
...
### `MarketingKnowledgeBase/agent/webhook_profile.py`

Added reusable Discord webhook identity loading for MarketingKnowledgeBase agents.
...
```text
MarketingKnowledgeBase/agant-ai-discord-webhook-profile/avatar/rs_reese_avatar.png
```

The current `avatar_url` was sourced from the neo test Discord upload at:

```text
https://ptb.discord.com/channels/1451275225512546497/1517544158389207111/1517821897319125063
```

Discord webhook avatars require a public `https://` URL. The profile can store a local avatar asset path for deployment/reference, but runtime only sends `avatar_url` when a public URL is configured. If Discord's signed CDN link expires, replace `avatar_url` with a stable public hosted image URL and keep the local asset as backup.
...
- `agent.chat.webhook_username`: `Reese`
- `agent.webhook_profile.username`: `Reese`
- `agent.webhook_profile.path`: `agant-ai-discord-webhook-profile/profile.json`
...
- Reese recognizes the intent.
- Reese explains the workflow is reserved/not wired.
- Reese does not fabricate data.
...
Do not enable live GHL/SMS sending from Reese without explicit approval. Current behavior is draft-only.
